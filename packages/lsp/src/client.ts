 ...
 * LSP Client
 *
 * LSPServerManager singleton and LSPClient class for managing language server processes.
 * Uses Bun-native spawn API.
 */
...
import type { Diagnostic, LSPServerConfig, ResolvedServer } from "./types";
...
class LSPServerManager {
	...
	async getClient(root: string, server: ResolvedServer): Promise<LSPClient> {
		const key = this.getKey(root, server.config.id);
...
export class LSPClient {
	...
	async start(): Promise<void> {
		this.proc = spawn(this.server.config.command, {
			stdin: "pipe",
			stdout: "pipe",
			stderr: "pipe",
			cwd: this.root,
		...
		if (!this.proc) {
			throw new Error(`Failed to spawn LSP server: ${this.server.config.command.join(" ")}`);
		}
	...
	async initialize(): Promise<void> {
		...
		await this.send("initialize", {
			...
			...this.server.config.initializationOptions,
		});
